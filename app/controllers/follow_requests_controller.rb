class FollowRequestsController < ApplicationController
  ...
  def create
    ...
    if the_follow_request.valid?
      ...
      redirect_to("/users/#{User.where(:id => the_follow_request.recipient_id).at(0).username}", { :notice => "Follow request created successfully." })
      else
  ...
  def update
    ...
    if the_follow_request.valid?
      the_follow_request.save
      redirect_to("/follow_requests/#{the_follow_request.recipient.username}", { :notice => "Follow request updated successfully."} )
    else
